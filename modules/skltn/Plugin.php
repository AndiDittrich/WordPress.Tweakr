...
class Plugin{
    ...
    protected $_activationRedirectPage = 'admin.php?page=tweakr-about';
    ...
    public function _wp_init(){
        ...
        if (is_admin()){
            ...
            if (get_option('tweakr-activation-redirect', '') === 'about-page'){
                // remove redirect flag - do it twice to avoid infinite redirect issues with broken caching plugins!
                $deleted = update_option('tweakr-activation-redirect', '') && delete_option('tweakr-activation-redirect');
    ...
    public function setupBackend(){
        ...
        $renderTemplate = function($menuItem){
            ...
            include(TWEAKR_PLUGIN_PATH.'/views/admin/'.$menuItem['template'].'.phtml');
        };
    ...
    public function addPluginPageSettingsLink($links, $file){
        ...
        if ($file == 'tweakr/Tweakr.php'){
            ...
            if ($this->_pluginMetaSettingsPage !== null){
                $links[] = '<a href="'.admin_url('admin.php?page=tweakr-'.$this->_pluginMetaSettingsPage). '">'.__('Settings', 'tweakr').'</a>';
            }

            // about page link ?
            if ($this->_pluginMetaAboutPage !== null){
                $links[] = '<a href="'.admin_url('admin.php?page=tweakr-'.$this->_pluginMetaAboutPage). '">'.__('About', 'tweakr').'</a>';
            }
        }
        return $links;
    }
    ...
    public function _wp_plugin_activate(){
        // set activation flag
        add_option('tweakr-activation-redirect', 'about-page');
    }
    ...
    public function showUpgradeMessage(){
        ...
        echo '<strong>Tweakr Plugin Upgrade:</strong> The Plugin has been upgraded to <strong>2.1</strong>';
        echo '</p></div>';
    }
    ...
    public static function run($pluginName){
        ...
        if (self::$__instance==null){
            ...
            if ($version == '0.0.0'){
                // store new version
                update_option('tweakr-version', '2.1');

            // plugin upgraded ?
            }else if (version_compare('2.1', $version, '>')){
                // run upgrade hook
