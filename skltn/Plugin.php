...
class Plugin{
    ...
    public function showUpgradeMessage(){
        ...
        echo '<strong>Tweakr Plugin Upgrade:</strong> The Plugin has been upgraded to <strong>2.0</strong>';
        echo '</p></div>';
    }
    ...
    public static function run($pluginName){
        ...
        if (self::$__instance==null){
            ...
            if ($version == '0.0.0'){
                // store new version
                update_option('tweakr-version', '2.0');

            // plugin upgraded ?
            }else if (version_compare('2.0', $version, '>')){
                // run upgrade hook
